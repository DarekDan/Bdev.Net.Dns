...
{
    /// <summary>Summary description for Dns.</summary>
    public static partial class Resolver
    {
        private const int DnsPort = 53;
        private const int UdpRetryAttempts = 3;
        ...
        public static MXRecord?[]? MXLookup(string? domain, IPAddress? dnsServer)
        {
            ...
            var resourceRecords = response.Answers
                .Where(w => w.Record is MXRecord)
                .Select(s => s.Record as MXRecord)
        ...
        {
            return Lookup(Request.Question(new Question(value, type)), dnsServer ?? DnsServers.IP4.First());
        }
        ...
        private static byte[] UdpTransfer(IPEndPoint server, byte[] requestMessage)
        {
            ...
            for (var attempts = 0; attempts < UdpRetryAttempts; attempts++) // try repeatedly in case of failure
            {
                ...
                catch (SocketException) { /* failure - we better try again */ }
            }
